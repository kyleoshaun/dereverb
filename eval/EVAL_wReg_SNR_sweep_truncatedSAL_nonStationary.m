...
rir_database = load('rir_databases_4micCombined_2noise.mat');
rir_database = rir_database.rir_databases_4micCombined_2noise;
...
results_dir = sprintf("FINAL_RESULTS_wReg/EVAL_SNR_Sweep_truncatedSAL_%.0fmsec_T60_%s_%s", T60*1000, noise_memo, datetime("today"));
run_num = 1;
while exist(results_dir, 'Dir')
results_dir = sprintf("FINAL_RESULTS_wReg/EVAL_SNR_Sweep_truncatedSAL_%.0fmsec_T60_%s_%s_%d", T60*1000, noise_memo, datetime("today"), run_num);
    run_num = run_num + 1;
end
mkdir(results_dir)
...
STMI_list_NH_unproc    = zeros(length(SNR_dB_list),1); 
NSIM_FT_list_NH_unproc = zeros(length(SNR_dB_list),1); 
NSIM_MR_list_NH_unproc = zeros(length(SNR_dB_list),1); 
HASPI_list_NH_unproc   = zeros(length(SNR_dB_list),1); 
STOI_list_NH_unproc    = zeros(length(SNR_dB_list),1); 
HASQI_list_NH_unproc   = zeros(length(SNR_dB_list),1); 
VISQOL_list_NH_unproc  = zeros(length(SNR_dB_list),1); 

STMI_list_HI_unproc    = zeros(length(SNR_dB_list),1); 
NSIM_FT_list_HI_unproc = zeros(length(SNR_dB_list),1); 
NSIM_MR_list_HI_unproc = zeros(length(SNR_dB_list),1); 
HASPI_list_HI_unproc   = zeros(length(SNR_dB_list),1); 
STOI_list_HI_unproc    = zeros(length(SNR_dB_list),1); 
HASQI_list_HI_unproc   = zeros(length(SNR_dB_list),1); 
VISQOL_list_HI_unproc  = zeros(length(SNR_dB_list),1); 

STMI_list_NH_proc    = zeros(length(SNR_dB_list),1); 
NSIM_FT_list_NH_proc = zeros(length(SNR_dB_list),1); 
NSIM_MR_list_NH_proc = zeros(length(SNR_dB_list),1); 
HASPI_list_NH_proc   = zeros(length(SNR_dB_list),1); 
STOI_list_NH_proc    = zeros(length(SNR_dB_list),1); 
HASQI_list_NH_proc   = zeros(length(SNR_dB_list),1); 
VISQOL_list_NH_proc  = zeros(length(SNR_dB_list),1); 

STMI_list_HI_proc    = zeros(length(SNR_dB_list),1); 
NSIM_FT_list_HI_proc = zeros(length(SNR_dB_list),1); 
NSIM_MR_list_HI_proc = zeros(length(SNR_dB_list),1); 
HASPI_list_HI_proc   = zeros(length(SNR_dB_list),1); 
STOI_list_HI_proc    = zeros(length(SNR_dB_list),1); 
HASQI_list_HI_proc   = zeros(length(SNR_dB_list),1); 
VISQOL_list_HI_proc  = zeros(length(SNR_dB_list),1); 

C50_list_unproc  = zeros(length(SNR_dB_list),1); 
C50_list_proc    = zeros(length(SNR_dB_list),1); 
...
for SNR_num = 1:length(SNR_dB_list)
    ...
    % Channel        
    if T60 == 0
        G_source_trunc = [ones(1,M) ; zeros(1,M)];sour
    else
    ...
    [metrics] = run_eval_wReg(source_data, ...
                         noise_data,  ...
                         interf_data, ...
                         fs, ...
                         N60, ...
    ...
    % Save to buffers
    STMI_list_NH_unproc(SNR_num)    = metrics.metrics_unproc_NH.STMI;
    NSIM_FT_list_NH_unproc(SNR_num) = metrics.metrics_unproc_NH.NSIM_FT;
    NSIM_MR_list_NH_unproc(SNR_num) = metrics.metrics_unproc_NH.NSIM_MR;
    HASPI_list_NH_unproc(SNR_num)   = metrics.metrics_unproc_NH.HASPI;
    STOI_list_NH_unproc(SNR_num)    = metrics.metrics_unproc_NH.STOI;
    HASQI_list_NH_unproc(SNR_num)   = metrics.metrics_unproc_NH.HASQI;
    VISQOL_list_NH_unproc(SNR_num)  = metrics.metrics_unproc_NH.VISQOL;

    STMI_list_HI_unproc(SNR_num)    = metrics.metrics_unproc_HI.STMI;
    NSIM_FT_list_HI_unproc(SNR_num) = metrics.metrics_unproc_HI.NSIM_FT;
    NSIM_MR_list_HI_unproc(SNR_num) = metrics.metrics_unproc_HI.NSIM_MR;
    HASPI_list_HI_unproc(SNR_num)   = metrics.metrics_unproc_HI.HASPI;
    STOI_list_HI_unproc(SNR_num)    = metrics.metrics_unproc_HI.STOI;
    HASQI_list_HI_unproc(SNR_num)   = metrics.metrics_unproc_HI.HASQI;
    VISQOL_list_HI_unproc(SNR_num)  = metrics.metrics_unproc_HI.VISQOL;

    STMI_list_NH_proc(SNR_num)    = metrics.metrics_proc_NH.STMI;
    NSIM_FT_list_NH_proc(SNR_num) = metrics.metrics_proc_NH.NSIM_FT;
    NSIM_MR_list_NH_proc(SNR_num) = metrics.metrics_proc_NH.NSIM_MR;
    HASPI_list_NH_proc(SNR_num)   = metrics.metrics_proc_NH.HASPI;
    STOI_list_NH_proc(SNR_num)    = metrics.metrics_proc_NH.STOI;
    HASQI_list_NH_proc(SNR_num)   = metrics.metrics_proc_NH.HASQI;
    VISQOL_list_NH_proc(SNR_num)  = metrics.metrics_proc_NH.VISQOL;

    STMI_list_HI_proc(SNR_num)    = metrics.metrics_proc_HI.STMI;
    NSIM_FT_list_HI_proc(SNR_num) = metrics.metrics_proc_HI.NSIM_FT;
    NSIM_MR_list_HI_proc(SNR_num) = metrics.metrics_proc_HI.NSIM_MR;
    HASPI_list_HI_proc(SNR_num)   = metrics.metrics_proc_HI.HASPI;
    STOI_list_HI_proc(SNR_num)    = metrics.metrics_proc_HI.STOI;
    HASQI_list_HI_proc(SNR_num)   = metrics.metrics_proc_HI.HASQI;
    VISQOL_list_HI_proc(SNR_num)  = metrics.metrics_proc_HI.VISQOL;

    C50_list_unproc(SNR_num) = metrics.metrics_physical_unproc.C50;
    C50_list_proc(SNR_num)   = metrics.metrics_physical_proc.C50;
...
subplot(1,2,1)
plot(SNR_dB_list, STMI_list_NH_unproc,    'Color', plot_colours(1,:), 'LineStyle', '--', 'Marker', 'o')
hold on;
plot(SNR_dB_list, STMI_list_NH_proc,      'Color', plot_colours(1,:), 'LineStyle', '-',  'Marker', 'o')
plot(SNR_dB_list, NSIM_FT_list_NH_unproc, 'Color', plot_colours(2,:), 'LineStyle', '--', 'Marker', '^')
plot(SNR_dB_list, NSIM_FT_list_NH_proc,   'Color', plot_colours(2,:), 'LineStyle', '-',  'Marker', '^')
plot(SNR_dB_list, NSIM_MR_list_NH_unproc, 'Color', plot_colours(3,:), 'LineStyle', '--', 'Marker', 'diamond')
plot(SNR_dB_list, NSIM_MR_list_NH_proc,   'Color', plot_colours(3,:), 'LineStyle', '-',  'Marker', 'diamond')
plot(SNR_dB_list, HASPI_list_NH_unproc,   'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
plot(SNR_dB_list, HASPI_list_NH_proc,     'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
plot(SNR_dB_list, STOI_list_NH_unproc,    'Color', plot_colours(5,:), 'LineStyle', '--', 'Marker', '+')
plot(SNR_dB_list, STOI_list_NH_proc,      'Color', plot_colours(5,:), 'LineStyle', '-',  'Marker', '+')
ylim([0 Inf])
xlabel('T60 (sec)')
...
subplot(1,2,2)
plot(SNR_dB_list, STMI_list_HI_unproc,    'Color', plot_colours(1,:), 'LineStyle', '--', 'Marker', 'o')
hold on;
plot(SNR_dB_list, STMI_list_HI_proc,      'Color', plot_colours(1,:), 'LineStyle', '-',  'Marker', 'o')
plot(SNR_dB_list, NSIM_FT_list_HI_unproc, 'Color', plot_colours(2,:), 'LineStyle', '--', 'Marker', '^')
plot(SNR_dB_list, NSIM_FT_list_HI_proc,   'Color', plot_colours(2,:), 'LineStyle', '-',  'Marker', '^')
plot(SNR_dB_list, NSIM_MR_list_HI_unproc, 'Color', plot_colours(3,:), 'LineStyle', '--', 'Marker', 'diamond')
plot(SNR_dB_list, NSIM_MR_list_HI_proc,   'Color', plot_colours(3,:), 'LineStyle', '-',  'Marker', 'diamond')
plot(SNR_dB_list, HASPI_list_HI_unproc,   'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
plot(SNR_dB_list, HASPI_list_HI_proc,     'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
ylim([0 Inf])
xlabel('T60 (sec)')
legend("STMI (unprocessed)",    "STMI (processed)", ...
       "NSIM FT (unprocessed)", "NSIM FT (processed)", ...
...
set(gcf,'Position',fig_size)
subplot(1,2,1)
plot(SNR_dB_list, STMI_list_NH_unproc .* scale_STMI,    'Color', plot_colours(1,:), 'LineStyle', '--', 'Marker', 'o')
hold on;
plot(SNR_dB_list, STMI_list_NH_proc .* scale_STMI,      'Color', plot_colours(1,:), 'LineStyle', '-',  'Marker', 'o')
plot(SNR_dB_list, NSIM_FT_list_NH_unproc .* scale_NSIM_FT, 'Color', plot_colours(2,:), 'LineStyle', '--', 'Marker', '^')
plot(SNR_dB_list, NSIM_FT_list_NH_proc .* scale_NSIM_FT,   'Color', plot_colours(2,:), 'LineStyle', '-',  'Marker', '^')
plot(SNR_dB_list, NSIM_MR_list_NH_unproc .* scale_NSIM_MR, 'Color', plot_colours(3,:), 'LineStyle', '--', 'Marker', 'diamond')
plot(SNR_dB_list, NSIM_MR_list_NH_proc .* scale_NSIM_MR,   'Color', plot_colours(3,:), 'LineStyle', '-',  'Marker', 'diamond')
plot(SNR_dB_list, HASPI_list_NH_unproc .* scale_HASPI,   'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
plot(SNR_dB_list, HASPI_list_NH_proc .* scale_HASPI,     'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
plot(SNR_dB_list, STOI_list_NH_unproc .* scale_STOI,    'Color', plot_colours(5,:), 'LineStyle', '--', 'Marker', '+')
plot(SNR_dB_list, STOI_list_NH_proc .* scale_STOI,      'Color', plot_colours(5,:), 'LineStyle', '-',  'Marker', '+')
ylim([0 Inf])
xlabel('SNR (dB)')
legend("STMI (unprocessed)",    "STMI (processed)", ...
       "NSIM FT (unprocessed)", "NSIM FT (processed)", ...
...
subplot(1,2,2)
plot(SNR_dB_list, STMI_list_HI_unproc .* scale_STMI,    'Color', plot_colours(1,:), 'LineStyle', '--', 'Marker', 'o')
hold on;
plot(SNR_dB_list, STMI_list_HI_proc .* scale_STMI,      'Color', plot_colours(1,:), 'LineStyle', '-',  'Marker', 'o')
plot(SNR_dB_list, NSIM_FT_list_HI_unproc .* scale_NSIM_FT, 'Color', plot_colours(2,:), 'LineStyle', '--', 'Marker', '^')
plot(SNR_dB_list, NSIM_FT_list_HI_proc .* scale_NSIM_FT,   'Color', plot_colours(2,:), 'LineStyle', '-',  'Marker', '^')
plot(SNR_dB_list, NSIM_MR_list_HI_unproc .* scale_NSIM_MR, 'Color', plot_colours(3,:), 'LineStyle', '--', 'Marker', 'diamond')
plot(SNR_dB_list, NSIM_MR_list_HI_proc .* scale_NSIM_MR,   'Color', plot_colours(3,:), 'LineStyle', '-',  'Marker', 'diamond')
plot(SNR_dB_list, HASPI_list_HI_unproc .* scale_HASPI,   'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
plot(SNR_dB_list, HASPI_list_HI_proc .* scale_HASPI,     'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
ylim([0 Inf])
xlabel('SNR (dB)')
legend("STMI (unprocessed)",    "STMI (processed)", ...
       "NSIM FT (unprocessed)", "NSIM FT (processed)", ...
...
set(gcf,'Position',fig_size)
subplot(1,2,1)
plot(SNR_dB_list, HASQI_list_NH_unproc,  'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
hold on;
plot(SNR_dB_list, HASQI_list_NH_proc,    'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
plot(SNR_dB_list, VISQOL_list_NH_unproc, 'Color', plot_colours(1,:), 'LineStyle', '--', 'Marker', 'diamond')
plot(SNR_dB_list, VISQOL_list_NH_proc,   'Color', plot_colours(1,:), 'LineStyle', '-',  'Marker', 'diamond')
ylim([0 ylim_SQ])
...
subplot(1,2,2)
plot(SNR_dB_list, HASQI_list_HI_unproc,  'Color', plot_colours(4,:), 'LineStyle', '--', 'Marker', 'square')
hold on;
plot(SNR_dB_list, HASQI_list_HI_proc,    'Color', plot_colours(4,:), 'LineStyle', '-',  'Marker', 'square')
xlabel('SNR (dB)')
legend("HASQI (unprocessed)", "HASQI (processed)")
...
figure()
%set(gcf,'Position',fig_size)
plot(SNR_dB_list, 10*log10(C50_list_unproc), "--square")
hold on;
plot(SNR_dB_list, 10*log10(C50_list_proc), "-square")
xlabel('SNR (dB)')
ylabel('dB')
