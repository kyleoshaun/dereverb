...
T60_max = 2.5;
N60_max = T60_max * fs;
G_source = randn(1.5*N60_max, M);
% Exponential decay curve
%L_channel = min(round(N60*1.25), length(G_source(:,1)));
L_channel = length(G_source(:,1));
tau       = N60_max / log(10^(60 / 20)); % -ln(10^(-60/20)) = ln(10^3)
...
% random uncorrelated spectral shapes for RTFs (Exp decaying gaussians are white)
%random_RTF_colour = randn(3, M);
%for ch=1:M
%    G_source(:,ch) = filter(random_RTF_colour(:,ch), 1, G_source(:,ch));
%end

% Increase magnitude of direct sound
for ch=1:M
    while abs(G_source(1,ch)) < 1
        G_source(1,ch) = G_source(1,ch) .* 2;
    end
    G_source(1,ch) = G_source(1,ch) * 4;
    %G_source(1:20,ch) = G_source(1:20,ch) .* 50;
    %G_source(21:6000,ch) = G_source(21:6000,ch) .* 20;
end

% Synthetic time delay
%synth_delays = 0:(M-1);
synth_delays = zeros(M,1);
for ch = 1:M
...
for ch = 1:M
    subplot(M,1,ch)
    plot((0:(length(G_source(:,ch))-1)) .* (1/fs), 10*log10(EDC(G_source(:,ch))))
    grid on;
    xlabel('Time [sec]')
    title(sprintf("EDC: Synthetic RIR Channel %d", ch))
end
...
results_dir = sprintf("FINAL_RESULTS/EVAL_T60_sweep_synth_%s", datetime("today"));
run_num = 1;
while exist(results_dir, 'Dir')
    results_dir = sprintf("FINAL_RESULTS/EVAL_T60_sweep_synth_%s_%d", datetime("today"), run_num);
    run_num = run_num + 1;
end
mkdir(results_dir)
    ...
    for T60_num = 1:length(T60_list)
        ...
        if T60 == 0
            ...
            for ch = 1:M
                ...
                % % TEMP!!!!!
                % % Apply additional decay to late decay region
                % N60_new = (500/1000)*fs;
                % N_early_decay = (250/1000) * fs;
                % N60_late_decay = N60_new - N_early_decay;
                % added_late_decay_dB = 60-15;
                % L_channel = min(round(N60*2), length(G_source(:,1)));
                % tau_late_decay = N60_late_decay / log(10^(added_late_decay_dB / 20)); % -ln(10^(-60/20)) = ln(10^3)
                % late_decay = [ones(N_early_decay-1,1) ; exp(-1 .* (0:(L_channel-N_early_decay))' ./ tau_late_decay)];
                % % %temp_win_length = (500/1000)*fs;
                % % %temp_window = [ones(temp_win_length,1) ; 0.1 .* ones((length(G_source_trunc(:,ch))-temp_win_length),1)];
                % % figure()
                % % subplot(3,1,1)
                % % plot((0:(length(G_source_trunc(:,ch))-1)) .* (1/fs), 10*log10(EDC(G_source_trunc(:,ch))))
                % % grid on
                % % subplot(3,1,2)
                % % plot((0:(length(G_source_trunc(:,ch))-1)) .* (1/fs), 20*log10(late_decay))
                % % grid on
                % 
                % G_source_trunc(:,ch) = G_source_trunc(:,ch) .* late_decay;
                % % subplot(3,1,3)
                % % plot((0:(length(G_source_trunc(:,ch))-1)) .* (1/fs), 10*log10(EDC(G_source_trunc(:,ch))))
                % % grid on  
...
test_memo_NH   = sprintf("(Synthetic RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL, HL = [%.0f %.0f %.0f %.0f %.0f %.0f], No HA Gain)", SNR_dB, stimdb, HL_NH(1), HL_NH(2), HL_NH(3), HL_NH(4), HL_NH(5), HL_NH(6));
test_memo_HI   = sprintf("(Synthetic RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL, HL = [%.0f %.0f %.0f %.0f %.0f %.0f], NAL-R Gain)", SNR_dB, stimdb, HL_HI(1), HL_HI(2), HL_HI(3), HL_HI(4), HL_HI(5), HL_HI(6));
test_memo_phys = sprintf("(Synthetic RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL)", SNR_dB, stimdb);
