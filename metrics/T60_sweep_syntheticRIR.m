...
results_dir = sprintf("Results_T60Sweep_Synthetic_%.0fdB_SNR_FIXED_dBSPL", SNR_dB);
...
L_channel_max = round(max(T60_list) * fs * 1.5);
g_rir_max = randn(L_channel_max,1);
...
for T60_num = 1:length(T60_list)
    ...
    if T60 == 0
        ...
        g_rir = g_rir_max(1:L_channel) .* exp_decay;
...
test_memo_HI = sprintf("(Synthetic RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL, HL = [%.0f %.0f %.0f %.0f %.0f %.0f], NAL-R HA Gain)", SNR_dB, stimdb, HL_HI(1), HL_HI(2), HL_HI(3), HL_HI(4), HL_HI(5), HL_HI(6));
