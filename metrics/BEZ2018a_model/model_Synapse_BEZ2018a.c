...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *px, cf, tdres, tabs, trel, spont;
    int    nrep, pxbins, lp, totalstim, noiseType, implnt, expliketype;
    mwSize  outsize[2];
    ...
    noiseType  = (int)noiseTypetmp[0];  /* fixed or variable fGn */

    implnt = (int)implnttmp[0];  /* actual/approximate implementation of the power-law functions */
    ...
    expliketype = (int)expliketmp[0];  /* type of  exponential-like function used in Vihc -> synapse mapping*/
    if (expliketype==0)
        mexPrintf("note: old IHC -> synapse mapping function selected\nnote: new softplus function is preferred\n");
    if (expliketype==2)
        mexPrintf("warning: expontential IHC -> synapse mapping function selected\nwarning: not for general use!\n");
    if (expliketype==3)
        mexPrintf("warning: Boltzmann IHC -> synapse mapping function selected\nwarning: not for general use!\n");
...
/* --------------------------------------------------------------------------------------------*/
double Synapse(double *ihcout, double tdres, double cf, int totalstim, int nrep, double spont, int noiseType, int implnt, int expliketype, double sampFreq, double *synout)
{
    /* Initalize Variables */
    int z, b;
    ...
    double *sout1, *sout2, *synSampOut, *powerLawIn, *mappingOut, *TmpSyn, *explikeOut;
    double *m1, *m2, *m3, *m4, *m5;
    double *n1, *n2, *n3;

    double p1sp = 0.00172;
    double p2sp = 1165; /* Values based on Zhang et al. (2001) */

    /* double p1sp = 0.00025;
    double p2sp = 8000; */ /* Alternative values the get a stronger nonlinearity with thresholds around 0 dB SPL */

    double p1exp = 0.001268;
    double p2exp = 747.9; /* Values for exp function, based on fit to function from Zhang et al. (2001) around zero */

    double p1bltz = 787.77;
    double p2bltz = 749.69; /* Values for Boltzmann function, based on fit to function from Zhang et al. (2001) around zero */
    ...
    explikeOut = (double*)mxCalloc((long) ceil(totalstim*nrep),sizeof(double));
    ...
    *mxGetPr(randInputArray[4])= spont; /* high, medium, or low */
    ...
    mexCallMATLAB(1, randOutputArray, 5, randInputArray, "ffGn_2018a");
    ...
    switch (expliketype)
    {
        ...
        case 0: /* No expontential-like function included IHC -> mapping */

            for (k=0; k<totalstim*nrep; ++k)
            {
        ...
        case 1: /* shifted softplus function included IHC -> mapping */

            for (k=0; k<totalstim*nrep; ++k)
            {

                explikeOut[k] = p1sp*(log(1.0+exp(p2sp*ihcout[k]))-log(2.0));
                if (isfinite(explikeOut[k]) < 1) /* if expOut is infinite, then replace with finite value */
                    explikeOut[k] = p1sp*(p2sp*ihcout[k]-log(2.0)); /* linear asymptote */

                mappingOut[k] = pow(10,(0.9*log10(fabs(explikeOut[k])*cf_factor))+ multFac);
        ...
        case 2: /* shifted exponential function included IHC -> mapping */

            for (k=0; k<totalstim*nrep; ++k)
            {

                explikeOut[k] = p1exp*(exp(p2exp*ihcout[k])-1.0);
                if (explikeOut[k] > 30) /* if expOut is too large, then replace with reasonable finite */
                    explikeOut[k] =30; /* reasonable finite value */
        ...
        case 3: /* shifted Boltzmann function included IHC -> mapping */

            for (k=0; k<totalstim*nrep; ++k)
            {

                explikeOut[k] = 1.0/(1.0+p1bltz*exp(-p2bltz*ihcout[k]))-1.0/(1.0+p1bltz);
    ...
    *mxGetPr(IhcInputArray[2])= resamp;
    ...
    mxFree(powerLawIn); mxFree(mappingOut); mxFree(explikeOut);
    /*----------------------------------------------------------*/
