function [neurogram_ft,neurogram_mr,neurogram_Sout,t_ft,t_mr,t_Sout,CFs] = generate_neurogram_BEZ2018a_parallelized(stim,Fs_stim,species,ag_fs,ag_dbloss)
...
parfor CFlp = 1:numcfs
    ...
    for spontlp = 1:sum(numsponts)
        ...
        [psth_ft,meanrate,varrate,synout] = model_Synapse_BEZ2018a(vihc,CF,nrep,1/Fs,noiseType,implnt,spont,tabs,trel,expliketype);
        psthbins = round(psthbinwidth_mr*Fs);  % number of psth_ft bins per psth bin
        psth_mr = sum(reshape(psth_ft,psthbins,length(psth_ft)/psthbins));
...
t_ft = 0:ceil(windur_ft/2)/Fs:(size(neurogram_ft,2)-1)*ceil(windur_ft/2)/Fs; % time vector for the fine-timing neurogram
neurogram_mr = neurogram_mr(:,1:ceil(windur_mr/2):end); % 50% overlap in Hamming window
t_mr = 0:ceil(windur_mr/2)*psthbinwidth_mr:(size(neurogram_mr,2)-1)*ceil(windur_mr/2)*psthbinwidth_mr; % time vector for the mean-rate neurogram
