...
results_dir = sprintf("Results_T60Sweep_Real_%.0fdB_SNR", SNR_dB);
...
for T60_num = 1:length(T60_list)
    ...
    % Channel
    channel_memo = sprintf("Real RIR (%s)", room_memos{T60_num});
        
    G_source    = RIR_list_0deg{T60_num};
    g_rir       = (G_source(:,1));
    L_channel   = length(g_rir);
...
test_memo_NH = sprintf("(Real RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL, HL = [%.0f %.0f %.0f %.0f %.0f %.0f], No HA Gain)", SNR_dB, stimdb, HL_NH(1), HL_NH(2), HL_NH(3), HL_NH(4), HL_NH(5), HL_NH(6));
test_memo_HI = sprintf("(Real RIR, SNR = %.0f dB, Stimulus = %.0f dBSPL, HL = [%.0f %.0f %.0f %.0f %.0f %.0f], NAL-R HA Gain)", SNR_dB, stimdb, HL_HI(1), HL_HI(2), HL_HI(3), HL_HI(4), HL_HI(5), HL_HI(6));
